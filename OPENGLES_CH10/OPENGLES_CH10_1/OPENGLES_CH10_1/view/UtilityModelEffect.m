...
{
    glBindAttribLocation(self.program, UtilityVertexAttribPosition, "a_position");
    glBindAttribLocation(self.program, UtilityVertexAttribNormal, "a_normal");
    glBindAttribLocation(self.program, UtilityVertexAttribTexCoord0, "a_texCoords0");
}
...
// This method exists a minor optimization to update the light colors matrix without updating any other uniform values used by the shading language program;
...
// This method exist as a minor optimization to update modelview matrixwithout updating any other uniform values used by the Shading Language program.
-(void)prepareModelviewWithoutNormal
{
    ...
    glUniformMatrix4fv(uniforms[TEModelMVPMatrix], 1, 0, modelViewProjectionMatrix.m);
}

// This method exists as a minor optimization to update the modelview matrix and normal matrix without updating any other uniform values used by the Shading Language program.
-(void)prepareModelview
{
    // Pre-calculate the mvpMatrix and normal matrix
    GLKMatrix4 modelViewProjectionMatrix = GLKMatrix4Multiply(self.projectionMatrix, self.modelviewMatrix);
    glUniformMatrix4fv(uniforms[TEModelMVPMatrix], 1, 0, modelViewProjectionMatrix.m);
    GLKMatrix3 normalMatrix = GLKMatrix4GetMatrix3(GLKMatrix4InvertAndTranspose(self.modelviewMatrix, NULL));
    glUniformMatrix3fv(uniforms[TEModelNormalMatrix], 1, GL_FALSE, normalMatrix.m);
}
...
-(void)updateUniformValues
{
    ...
    glUniform1iv(uniforms[TEModelSamplers2D], MAX_TEXTURES, samplerIDs);
    
    // Lighting
    glUniform3fv(uniforms[TEModelDiffuseLightDirection], 1, self.diffuseLightDirection.v);
