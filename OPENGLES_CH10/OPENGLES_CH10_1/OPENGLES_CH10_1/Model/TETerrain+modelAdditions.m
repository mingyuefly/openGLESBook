...
// This function returns a pointer to a vector storing the X,Y,Z coordinates the vertex corresponding to the x,z position in terrain.
static inline const GLKVector3 *TETerrainPositionPtrAt(TETerrain *terrain,
                                                       ...
                                                       NSInteger z)
{
    ...
    return ((const GLKVector3 *)[positionAttributes bytes]) + (z * terrain.width + x);
}

// Return the calculated height of the terrain in meters at position x,z in terrain mesh coordinates. Use this method when height at the nearest terrain vertex isn't precise enough.
                   ...
                   surfaceNormal:(GLKVector3 *)aNormal
{
    ...
    GLKVector3 b = {floorf(x), 0.0f, ceil(z + 0.0001)};
    ...
    } else if (AGLKRayDoesIntersectTriangle(rayDirection,
                                            p,
                                            d,
                                            b,
                                            c,
...
// Return the calculated height of the terrain in meters at position x,z in meters. Use this method when height at the nearest terrain mesh vetex isn't precise enough.
-(GLfloat)calculatedHeightAtXPosMeters:(GLfloat)x
                            zPosMeters:(GLfloat)z
                         surfaceNormal:(GLKVector3 *)aNormal
{
    GLfloat matersPerUnit = self.metersPerUnit;
    x = x / matersPerUnit;
    z = z / matersPerUnit;
...
// Return the height of the terrain in meters at position x,z in terrain mesh coordinates.
-(GLfloat)heightAtXPosMeters:(GLfloat)x
                  zPosMeters:(GLfloat)z
{
    GLfloat metersPerUnit = self.metersPerUnit;
    x = x / metersPerUnit;
...
// Return the highest height in meters of the terrain near position x,z.
    ...
    for (zIndex = z - 1; zIndex <= z + 1; zIndex++) {
        for (xIndex = x - 1; xIndex <= x + 1; xIndex++) {
            if ([self isHeightValidAtXPos:xIndex zPos:zIndex]) {
                maxNearbyHeight = MAX(maxNearbyHeight, [self heightAtXPos:xIndex zPos:zIndex]);
            }
        }
    }
...
// Returns YES if and only if a valid height value is available for the terrain at position x,z
-(BOOL)isHeightValidAtXPos:(NSInteger)x
                      zPos:(NSInteger)z
{
...
// Returns the height in meters of the terrain at the specified location in meters.
...
// Return the average of the up to 9 mesh vertices centered at x,z.
                          ...
                          zPosMeters:(NSInteger)z
{
    ...
    NSInteger xIndex;
    GLfloat count = 0.0f;
    GLfloat heightSum = 0.0f;
    GLfloat heightAverage = 0.0f;
...
// Returns the width of the terrain in meters based on the receiver's metersPerUnit property.
-(GLfloat)widthMeters
{
    return self.width * self.metersPerUnit;
}

// Returns the maximum height of the terrain in meters based on the receiver's metersPerUnit property.
-(GLfloat)heightMeters
{
    return self.heightScaleFactor * self.metersPerUnit;
}

// Returns the length of the terrain in meters based on the receiver's metersPerUnit property.
