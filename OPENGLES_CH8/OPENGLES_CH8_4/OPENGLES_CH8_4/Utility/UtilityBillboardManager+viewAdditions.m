...
// Sort particles in reverse order for rendering from furthest to nearest.
                  ...
                  upVector:(GLKVector3)upVector
{
    ...
    // Calculate displacements for billboard vertices based on frustum. Billboards always face the viewer.
    ...
    if (self.shouldRenderSpherical) {
        // Recalculate up vector so that particles will be parallel to frustum's near plane.
        upUnitVector = GLKVector3CrossProduct(lookDirection, rightVector);
    }
    ...
    for (UtilityBillboard *billboard in [self.sortedBillboards reverseObjectEnumerator]) {
        if (0 <= billboard.distanceSquared) {
            // Due to sort order, remaining particles are behind the viewer so stop processing now.
            break;
        } else {
            // Billboard in front of viewer. Calculate vertex positions based on displacements.
            ...
            // Store vertices for two triangles that compose one billboard.
