...
{
    NSParameterAssert(anIndex < self.numberOfParticles);
    AGLKParticleAttributes *particlePtr = (AGLKParticleAttributes *)[self.particleAttributesData mutableBytes];
    ...
    if (0 != program) {
        ...
        if (self.particleDataWasUpdated) {
            if (nil == self.particleAttributeBuffer && 0 < [self.particleAttributesData length]) {
                // vertex attributes haven't been sent to GUP yet
                self.particleAttributeBuffer =
                [[AGLKVertexAttribArrayBuffer alloc]
                 initWithAttribStride:sizeof(AGLKParticleAttributes)
                 numberOfVertices:[self.particleAttributesData length]/sizeof(AGLKParticleAttributes)
                 bytes:[self.particleAttributesData bytes]
                 usage:GL_DYNAMIC_DRAW];
            } else {
                [self.particleAttributeBuffer
                 reinitWithAttribStride:sizeof(AGLKParticleAttributes)
                 numberOfVertices:[self.particleAttributesData length]/sizeof(AGLKParticleAttributes)
                 bytes:[self.particleAttributesData bytes]];
            }
...
-(void)draw
{
    glDepthMask(GL_FALSE);    // Disable depth buffer writers
    [self.particleAttributeBuffer drawArrayWithMode:GL_POINTS
                                   startVertexIndex:0
                                   numberOfVertices:self.numberOfParticles];
    glDepthMask(GL_TRUE);     // Reenable depth buffer writers
}
...
// Default gravity acceleration vector matches Earth's {0, (-9.80665 m/s/s), 0} assuming +Y up coordinate system
