...
-(void)layoutSubviews
{
    ...
    //Initialize the current Frame Buffer's pixel color buffer so that it shares the corresponding Core Animation Layer's pixel color storage.
    //调整视图的缓存的尺寸以匹配层的新尺寸
    ...
    if (self.drawableDepthFormat != AGLKViewDrawableDepthFormatNone &&
        ...
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, currentDrawableWidth, currentDrawableHeight);       //step3
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthRenderBuffer);                                 //step4
    }
