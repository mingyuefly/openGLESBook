...
// The number of indices is the number of triangles in mesh plus 2 plus number degenerate triangles (NUM_MESH_COLUMNS - 2)
...
// Designed intializer: creates vertex attribute array and initiallizes indices for sharing triangle vertices in mesh.
-(instancetype)init
{
    ...
    NSData *someIndexData = [NSData dataWithBytes:meshIndices length:sizeof(meshIndices)];
    
    return [self initWithVertexAttributeData:someMeshData indexData:someIndexData];
}

// Draw the entire mesh after it has been prepared for drawing
-(void)drawEntireMesh
{
    // Draw triangles using vertices in the prepared vertex buffers and indices from the bound element array buffer
    glDrawElements(GL_TRIANGLE_STRIP, NUM_MESH_INDICES, GL_UNSIGNED_SHORT, (GLushort *)NULL);
}

// Revert to default vertex attributes
...
// This method modifies vertex positions and recalculates normals. The vertex attribute array is reinitialized with the modified vertex data in this class' -draw method.
-(void)updateMeshWithElapsedTime:(NSTimeInterval)anInterval
{
    ...
    for (currentColumn = 0; currentColumn < NUM_MESH_COLUMNS; currentColumn++) {
        ...
        const GLfloat yOffset = 2.0f * sinf(M_PI * (phase + phaseOffset));
...
// Revert mesh to default vertex attributes
void SceneMeshUpdateMeshWithDefaultPositions(SceneMeshVertex mesh[NUM_MESH_COLUMNS][NUM_MESH_ROWS])
{
    ...
    // For each position along +X axis of mesh
    for (currentColum = 0; currentColum < NUM_MESH_COLUMNS; currentColum++) {
        // For each position along -Z axis of mesh
        for (currentRow = 0; currentRow < NUM_MESH_ROWS; currentRow++) {
            mesh[currentColum][currentRow].position = GLKVector3Make(currentColum, 0.0f, -currentRow);
            GLKVector2 textureCoords = GLKVector2Make((float)currentRow/(NUM_MESH_ROWS - 1), (float)currentColum/(NUM_MESH_COLUMNS - 1));
            mesh[currentColum][currentRow].texCoords0 = textureCoords;
        }
    }
    SceneMeshUpdateNormals(mesh);
}

// Initialize the indices needed to draw the animatable mesh as one large triangle strip
void SceneMeshInitIndices(GLushort meshIndices[NUM_MESH_INDICES])
{
    ...
    // Start at 1 because algorithm steps one index at start
    currentMeshIndex = 1;
...
// Calculate smooth normal vectors by averaging the normal vectors of four planes adjacent to each vertex. Normal vectors must be recalculated every time the vertex positions change.
void SceneMeshUpdateNormals(SceneMeshVertex mesh[NUM_MESH_COLUMNS][NUM_MESH_ROWS])
{
    ...
    for (currentRow = 1; currentRow < (NUM_MESH_ROWS - 1); currentRow++) {
        for (currentColumn = 1; currentColumn < (NUM_MESH_COLUMNS - 1); currentColumn++) {
            ...
            // Store the average the face normal vectors of the four triangles that share the current vertex
