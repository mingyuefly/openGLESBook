                      ...
                      numberOfIndices:(size_t)countIndices
{
    ...
    // Accumulat vertex attributes into vertexAttributesData
...
// This method prepares the current OpenGLES 2.0 context for drawing with the receiver's vertex attributes and indices.
-(void)prepareToDraw
{
    if (nil == self.vertexAttributeBuffer && 0 < [self.vertexData length]) {
        // vertex attributes haven't been sent to GPU yet
...
// Draw the entire mesh after it has been prepared for drawing. This method doesn't use vertex element indexing. Vertices in the range first to (first + count - 1) are drawn in order using mode.
...
// This method sends count sets of vertex attributes read from someVerts to the GPU. This method also marks the resulting vertex attribute array as a dynamic array prone to frequent updates.
