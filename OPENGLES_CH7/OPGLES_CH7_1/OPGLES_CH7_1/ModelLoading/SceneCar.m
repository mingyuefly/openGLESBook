...
-(instancetype)initWithModel:(UtilityModel *)aModel position:(GLKVector3)aPosition velocity:(GLKVector3)aVelocity color:(GLKVector4)aColor
{
    self = [super init];
    if (self) {
...
// This method detects any collisions between the receiver and walls in the simulation. Collisions case the receiver to "bounce off" the wall that was hit.
...
// This method detects any collisions betten the receiver and any other cars in the simulation. Collisions make the receiver and the hit care "bounce off" each other.
        ...
        if (currentCar != self) {
            ...
            if ((2.0f * self.radius) > distance) {
                ...
                
                {
                    ...
                    // Update position based on velocity and time since last update
...
// This method updates the receiver's current yaw angle to be closer to the receiver's target yaw angle.
-(void)spinTowarDirectionOfMotion:(NSTimeInterval)elapsed
{
    self.yawRadians = SceneScalarSlowLowPassFilter(elapsed, self.targetYawRadians, self.yawRadians);
}

// This method updates the position, yaw angle, and velocity of the receiver to simulate effects of collision with walls or other cars.
-(void)updateWithController:(id<SceneCarControllerProtocol>)controller
{
    // Calculate elapsed time bounded between 1/100th sec. and half a second
    ...
    if (0.1 > GLKVector3Length(self.velocity)) {
        // Got so slow that direction is unreliable so launch in a new direction
        self.velocity = GLKVector3Make((random()/(0.5f * RAND_MAX)) - 1.0f, 0.0f, (random()/(0.5f * RAND_MAX)) - 1.0f);
    ...
    // The dot product is the cos() of the angle between two vectors: in this case, the default orientation of the car model and the car's velocity vector.
    float dotProduct = GLKVector3DotProduct(GLKVector3Normalize(self.velocity), GLKVector3Make(0.0f, 0, -1.0f));
    
    // Set the target yaw angle to match the car's direction of motion
    if (0.0f > self.velocity.x) {
    ...
    [self spinTowarDirectionOfMotion:elapsedTimeSeconds];
    self.position = self.nextPosition;
}

// Draw the reciver: This method sets anEffect's current material color to the receivers color, translates to the receiver's position, ratates to match the receiver's yaw angle, draws the receiver's model. This method restores the values of anEffect's properties to values in place when the method was called.
...
// This function returns a value between target and current. Call this function repeatedly to asymptotically return values closer to target:"ease in" to the target value.
GLfloat SceneScalarFastLowPassFilter(NSTimeInterval elapsed,//seconds elapsed since last call
                                     GLfloat target,//target value to approach
                                     GLfloat current)//current value
{
    // Constant 50.0f is an arbitrarily "large" factor
    return current + (50.0f * elapsed *(target - current));
}
...
// This function returns a vector between target and current. Call repeatedly to asymptotically return vectors closer to target: "ease in" to the target value.
