...
// Set the index of the matrix to use for one armature joint per vertex animation
...
//
-(UtilityMeshJointInfluence)closestJointsToPosition:(GLKVector3)position joints:(NSArray *)joints
{
    struct jointInfo {
        float jointIndex;
    ...
    for (NSUInteger i = 0; i < count; i++) {
        ...
        // Joint matrix indices are 1 based because matrix[0] is reserved to be identity(mvpMatrix in shader)
    ...
    // Normalizing scale the weigths proportionally
...
//
-(UtilityMeshJointInfluence)jointBelowPosition:(GLKVector3)position joints:(NSArray *)joints
{
    struct jointInfo {
        float jointIndex;
        float distanceSquared;
...
// Set the index of the matrix to use for blended armature animation.
-(void)automaticallySkinSmoothWithJoints:(NSArray *)joints
{
    ...
    for (NSUInteger i = self.indexOfFirstCommand; i < lastCommandIndex; i++) {
        ...
        NSDictionary *currentCommand = [self.mesh.commands objectAtIndex:i];
        const GLsizei numberOfIndices = (GLsizei)[[currentCommand objectForKey:UtilityMeshCommandNumberOfIndices] unsignedIntegerValue];
        numberOfCommands_ = (GLsizei)[[currentCommand objectForKey:UtilityMeshCommandFirstIndex] unsignedIntegerValue];
        
        GLsizei lastIndex = lastIndex + numberOfIndices;
        
        for (GLsizei j = lastIndex; j < lastIndex; j++) {
            // For each mesh index used by command
            UtilityMeshVertex currentVertex = [self.mesh vertexAtIndex:j];
            UtilityMeshJointInfluence influences = [self closestJointsToPosition:currentVertex.position joints:joints];
            [self.mesh setJointInfluence:influences atIndex:j];
        }
    }
}
...
-(void)automaticallySkinRigidWithJoints:(NSArray *)joints
{
    ...
    for (NSUInteger i = self.indexOfFirstCommand; i < lastCommandIndex; i++) {
        ...
        for (GLsizei j = firstIndex; j < lastIndex; j++) {
            //For each mesh index used by command
            UtilityMeshVertex currentVertex = [self.mesh vertexAtIndex:j];
            UtilityMeshJointInfluence influences = [self jointBelowPosition:currentVertex.position joints:joints];
            [self.mesh setJointInfluence:influences atIndex:j];
        }
    }
}
