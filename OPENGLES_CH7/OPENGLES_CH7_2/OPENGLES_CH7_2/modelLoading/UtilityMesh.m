...
// Initialize the receiver with values for the skys, "vertexAttributeData", "indexData", and "commands" in aDictionary;
-(instancetype)initWithPlistRepresentation:(NSDictionary *)aDictionary
{
    self = [self init];
    if (self) {
        [mutableVertexData_ appendData:[aDictionary objectForKey:@"vertexAttributeData"]];
        [mutableIndexData_ appendData:[aDictionary objectForKey:@"indexData"]];
        commands_ = [commands_ arrayByAddingObjectsFromArray:[aDictionary objectForKey:@"commands"]];
    }
    return self;
}

// Deallocate OpenGL resources used by receivers.
-(void)dealloc
{
...
// Returns a dictionary storing "vertexAttributeData", "indexData", and "commands" keys with associated values.
-(NSDictionary *)plistRepresentation
{
    return @{@"vertexAttributeData":self.mutableVertexData,
             @"indexData":self.mutableIndexData,
             @"commands":self.commands
             };
}

// Returns a mutable data object suitable for storing extra attributes per vertex. The specific extra attributes depend on the needs of applications.
...
// Returns the calculate number of vertices stored by the receiver.
-(NSUInteger)numberOfVertices
{
    return [self.vertexData length]/sizeof(UtilityMeshVertex);
}

// Returns the calculated number of vertices stored by the receiver.
...
// Returns the calculated number of vertex indices stored by the receiver;
...
// Returns the receiver's vertex data. Vertices are type UtilityMeshVertex.
-(NSData *)vertexData
{
    return self.mutableVertexData;
}

// Return's the receiver's vertex at the specifixed index.
...
// This method returns a string encoding the minimum and maximum extents of an axis aligned bounding box enclosing all of the receiver's vertices described by the receiver's commands in the specified range.
...
// This method returns a string encoding the minimum and maximum extents of an axis aligned bounding box enclosing all of the receiver's vertices.
