...
@property (nonatomic, assign) GLKVector3 light1EypeDirection;
...
-(instancetype)init
{
    ...
    if (self) {
        ...
        self.mvpArmatureJointMatrices = calloc(sizeof(GLKMatrix4), MAX_INDEXED_MATRICES);
        self.normalArmatureJointNormalMatrices = calloc(sizeof(GLKMatrix3), MAX_INDEXED_MATRICES);
...
-(void)prepareToDrawArmature
{
    ...
    for (UtilityJoint *joint in self.jointsArray) {
        // Set the corresponding mvpArmatureJointMatrix to the joint matrix concatented with the mvp matrix.
        if (jointMatrixIndex < MAX_INDEXED_MATRICES) {
            GLKMatrix4 tempMatrix = joint.cumulativeTransforms;
            
            self.mvpArmatureJointMatrices[jointMatrixIndex] = GLKMatrix4Multiply(modelViewProjectionMatrix, tempMatrix);
            
            // Calculate normal matrix for the aramture matrix
            bool isInvertible;
            GLKMatrix3 jointNormalMatrix = GLKMatrix4GetMatrix3(GLKMatrix4InvertAndTranspose(tempMatrix, &isInvertible));
            
            if (isInvertible) {
                jointNormalMatrix = GLKMatrix3Multiply(self.transform.normalMatrix, jointNormalMatrix);
            } else {
    ...
    if (0 != _program) {
        ...
        glUniformMatrix4fv(_uniforms[AGLKNormalMatrix], 1, GL_FALSE, self.transform.normalMatrix.m);
        ...
        glUniform1iv(_uniforms[AGLKSamplers2D], 2, (const GLuint *)samplerIDs);
        
        // Precalculate the global ambient light contribution using only uniform parameters than send all the separate uniforms to the vertex shader
        ...
        // Light0
        // Material diffuse interaction with light is baked in so there is no reason to send material diffuse color to shaders
        if (self.light0.enabled) {
        ...
        // Armature matrices
        glUniformMatrix4fv(_uniforms[AGLKMVPJointMatrices], MAX_INDEXED_MATRICES, GL_FALSE, self.mvpArmatureJointMatrices[0].m);
        glUniformMatrix3fv(_uniforms[AGLKNormalJointNormalMatrices], MAX_INDEXED_MATRICES, GL_FALSE, self.normalArmatureJointNormalMatrices[0].m);
        
        // Bind all of the texture to their respective unites
...
-(void)setLight1SpotDirection:(GLKVector3)light1SpotDirection
{
    ...
    self.light1EyePosition = GLKVector3Normalize(GLKVector3Make(light1SpotDirection.x, light1SpotDirection.y, light1SpotDirection.z));
}
