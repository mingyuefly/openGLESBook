...
-(void)prepareToDraw
{
    ...
    if (0 != indexBufferID_) {
        ...
        // Indices haven't been sent out GPU yet
        // Create an element array buffer for mesh indices
...
// This method prepares the current OpenGL ES 2.0 context for picking with the receiver's vertex attributes and indices.
-(void)prepareToPick
{
    if (0 == vertexBufferID_ && 0 < [self.vertexData length]) {
        // Vertices haven't been sent to GUP yet
        glGenBuffers(1, &vertexBufferID_);
        NSAssert(0 != vertexBufferID_,
                 @"Failed to generate vertex array buffer");
...
// After the receiver has been prepared for drawing or picking, call this method to draw the positions of the mesh described by the specified range of the receiver's commands.
...
// After the receiver has been prepared for drawing, call this method to draw lines defining a box containing all portions of the mesh described by the specified range of the receiver's commands. This provides a quick visual way to see the volume occupied by portions of the mesh.
-(void)drawBoundingBoxStringForCommandsInRange:(NSRange)aRange
{
    if (0 < aRange.length) {
        ...
        for (NSUInteger i = aRange.location; i <= lastCommandIndex; i++) {
            ...
            glDrawElements(GL_LINE_STRIP, (GLsizei)numberOfIndices, GL_UNSIGNED_SHORT, indices + firstIndex);
        }
    }
}
